...
-- ── 4. Research runs ─────────────────────────────────────────────────────────
INSERT INTO research_runs (id, brand_id, status, sources_scanned, items_found, retriever_stats, started_at, completed_at)
VALUES
  (run_id_1, demo_brand_id, 'completed', 4, 24,
   '{"rss": {"found": 8, "latency_ms": 1200}, "serper": {"found": 9, "latency_ms": 2100}, "youtube": {"found": 4, "latency_ms": 3800}, "duckduckgo": {"found": 3, "latency_ms": 1320}}'::jsonb,
   now() - interval '2 days' - interval '8 seconds', now() - interval '2 days'),
  (run_id_2, demo_brand_id, 'completed', 4, 31,
   '{"rss": {"found": 10, "latency_ms": 980}, "serper": {"found": 12, "latency_ms": 1870}, "tavily": {"found": 6, "latency_ms": 2340}, "competitor": {"found": 3, "latency_ms": 3960}}'::jsonb,
   now() - interval '1 day' - interval '9 seconds', now() - interval '1 day');
...
VALUES
  (item_1, demo_brand_id, run_id_2,
   ...
   'TechCrunch', 'rss', 'trusted_source', 'approved', now() - interval '22 hours'),
  ...
  (item_2, demo_brand_id, run_id_2,
   ...
   'X (Twitter)', 'scrape', 'practitioner', 'approved', now() - interval '20 hours'),
  ...
  (item_3, demo_brand_id, run_id_2,
   ...
   'YouTube', 'youtube', 'practitioner', 'scored', now() - interval '18 hours'),
  ...
  (item_4, demo_brand_id, run_id_2,
   ...
   'Hacker News', 'search', 'keyword', 'approved', now() - interval '15 hours'),
  ...
  (item_5, demo_brand_id, run_id_2,
   ...
   'Product Hunt', 'scrape', 'trend', 'scored', now() - interval '12 hours'),
  ...
  (item_6, demo_brand_id, run_id_1,
   ...
   'Substack', 'rss', 'semantic', 'rejected', now() - interval '2 days'),
  ...
  (item_7, demo_brand_id, run_id_1,
   ...
   'Sequoia Capital', 'search', 'trusted_source', 'approved', now() - interval '2 days'),
  ...
  (item_8, demo_brand_id, run_id_1,
   ...
   'Competitor Blog', 'scrape', 'keyword', 'archived', now() - interval '2 days');

-- ── 6. Scores for research items ─────────────────────────────────────────────
-- Columns: applicability, credibility, alignment, trend_prediction, italy_relevance (all 0-10)
INSERT INTO scores (research_item_id, applicability, credibility, alignment, trend_prediction, italy_relevance, model_used, scoring_prompt_version, created_at)
VALUES
  (item_1, 9.1, 8.8, 9.0, 8.2, 7.5, 'claude-sonnet-4-6', 1, now() - interval '22 hours'),
  (item_2, 8.6, 7.9, 8.5, 9.4, 6.8, 'claude-sonnet-4-6', 1, now() - interval '20 hours'),
  (item_3, 8.2, 7.6, 8.0, 8.8, 7.0, 'claude-sonnet-4-6', 1, now() - interval '18 hours'),
  (item_4, 9.4, 8.5, 9.2, 7.8, 8.1, 'claude-sonnet-4-6', 1, now() - interval '15 hours'),
  (item_5, 7.4, 8.2, 7.6, 7.1, 6.5, 'claude-sonnet-4-6', 1, now() - interval '12 hours'),
  (item_6, 5.5, 9.0, 5.8, 6.0, 4.9, 'claude-sonnet-4-6', 1, now() - interval '2 days'),
  (item_7, 8.9, 9.5, 8.8, 8.5, 7.8, 'claude-sonnet-4-6', 1, now() - interval '2 days'),
  (item_8, 7.0, 6.5, 7.2, 5.5, 5.8, 'claude-sonnet-4-6', 1, now() - interval '2 days');
...
-- ── 9. Pipeline health ───────────────────────────────────────────────────────
INSERT INTO pipeline_health (brand_id, agent_name, status, uptime_pct, avg_latency_ms, errors_today, queue_size, last_heartbeat)
VALUES
  (demo_brand_id, 'researcher', 'healthy', 99.2, 1240, 0, 3, now() - interval '5 minutes'),
  (demo_brand_id, 'scorer', 'healthy', 100.0, 380, 0, 0, now() - interval '5 minutes'),
  (demo_brand_id, 'writer', 'healthy', 98.7, 2840, 1, 1, now() - interval '5 minutes'),
  (demo_brand_id, 'publisher', 'healthy', 99.8, 890, 0, 0, now() - interval '5 minutes');

-- ── 10. API costs ─────────────────────────────────────────────────────────────
INSERT INTO api_costs (brand_id, agent_name, model, operation, tokens_input, tokens_output, cost_usd, created_at)
VALUES
  (demo_brand_id, 'scorer', 'claude-sonnet-4-6', 'research_scoring', 12400, 2100, 0.0420, now() - interval '2 days'),
  (demo_brand_id, 'writer', 'claude-sonnet-4-6', 'content_generation', 8200, 4800, 0.0612, now() - interval '2 days'),
  (demo_brand_id, 'god_mode', 'claude-sonnet-4-6', 'god_mode_review', 6800, 3200, 0.0480, now() - interval '2 days'),
  (demo_brand_id, 'humanizer', 'claude-sonnet-4-6', 'humanizer', 4100, 1900, 0.0276, now() - interval '2 days'),
  (demo_brand_id, 'scorer', 'claude-sonnet-4-6', 'research_scoring', 14800, 2600, 0.0504, now() - interval '1 day'),
  (demo_brand_id, 'writer', 'claude-sonnet-4-6', 'content_generation', 9400, 5200, 0.0696, now() - interval '1 day'),
  (demo_brand_id, 'god_mode', 'claude-sonnet-4-6', 'god_mode_review', 7200, 3600, 0.0516, now() - interval '1 day'),
  (demo_brand_id, 'scorer', 'claude-sonnet-4-6', 'research_scoring', 11600, 2400, 0.0468, now()),
  (demo_brand_id, 'writer', 'claude-sonnet-4-6', 'content_generation', 7800, 4400, 0.0576, now());

-- ── 11. Calendar events ──────────────────────────────────────────────────────
INSERT INTO calendar_events (brand_id, draft_id, title, event_type, status, scheduled_date, scheduled_time)
VALUES
  (demo_brand_id, draft_1, 'LinkedIn: Open-source AI market', 'social', 'published', (now() - interval '2 days')::date, '09:00'),
  (demo_brand_id, draft_2, 'LinkedIn: HN open-source thread', 'social', 'confirmed', (now() + interval '1 day')::date, '09:30'),
  (demo_brand_id, draft_6, 'Newsletter #4 — Market map issue', 'newsletter', 'planned', (now() + interval '3 days')::date, '08:00'),
  (demo_brand_id, null, 'LinkedIn: GOD Mode architecture', 'social', 'planned', (now() + interval '5 days')::date, '09:00'),
  (demo_brand_id, null, 'X Thread: Full pipeline overview', 'social', 'planned', (now() + interval '7 days')::date, '10:00'),
  (demo_brand_id, null, 'Blog: Multi-tenancy deep dive', 'blog_video', 'planned', (now() + interval '10 days')::date, '11:00');
